# Angular Basic Training by @webia1

<!-- @import "[TOC]" {cmd="toc" depthFrom=1 depthTo=6 orderedList=false} -->

<!-- code_chunk_output -->

- [Angular Basic Training by @webia1](#angular-basic-training-by-webia1)
  - [Preconditions](#preconditions)
    - [Git](#git)
    - [Node/Npm](#nodenpm)
  - [Principles](#principles)
    - [Why Angular?](#why-angular)
      - [Alternatives](#alternatives)
    - [TypeScript](#typescript)
      - [Basics](#basics)
      - [Types (Basics)](#types-basics)
    - [Observables](#observables)
      - [Observables vs Promises](#observables-vs-promises)
    - [Angular API Reference](#angular-api-reference)
      - [Angular Cheat Sheet](#angular-cheat-sheet)
      - [Angular Coding Style Guide - Clean Code](#angular-coding-style-guide-clean-code)
      - [Angular Glossary](#angular-glossary)
  - [Getting Started](#getting-started)
    - [IDE - Visual Studio Code](#ide-visual-studio-code)
      - [Extensions](#extensions)
      - [Configuration](#configuration)
    - [Angular CLI](#angular-cli)
    - [Angular Configuration](#angular-configuration)
      - [Workspace](#workspace)
      - [TypeScript](#typescript-1)
    - [UI Libraries](#ui-libraries)
    - [Other Libraries](#other-libraries)
  - [Angular - Building Blocks](#angular-building-blocks)
    - [Components](#components)
    - [Templates](#templates)
    - [Directives](#directives)
    - [Routing & Navigation](#routing-navigation)
    - [Forms](#forms)
    - [API (HttpClient)](#api-httpclient)
    - [Modules](#modules)
    - [Lazy Loading](#lazy-loading)
  - [Angular Testing & Debugging](#angular-testing-debugging)
    - [Error Management](#error-management)
    - [Debugging](#debugging)
    - [Unit Tests / Integration Tests](#unit-tests-integration-tests)
    - [E2E Tests](#e2e-tests)
    - [API Tests](#api-tests)

<!-- /code_chunk_output -->

## Preconditions

### Git

### Node/Npm

### Fake-API-Setup

## Principles

### Why Angular?

#### Alternatives

### TypeScript

#### Basics

#### Types (Basics)

> See TypeScript/Types/ on this repo!

### Observables

#### Observables vs Promises

<https://angular.io/guide/comparing-observables>

### Angular API Reference

<https://angular.io/api>

#### Angular Cheat Sheet

<https://angular.io/guide/cheatsheet>

#### Angular Coding Style Guide - Clean Code

<https://angular.io/guide/styleguide>

#### Angular Glossary

<https://angular.io/guide/glossary>

## Getting Started

### IDE - Visual Studio Code

#### Extensions

#### Configuration

### Angular CLI

<https://angular.io/cli>

### Angular Configuration

#### Workspace

<https://angular.io/guide/file-structure>

#### TypeScript

<https://angular.io/guide/file-structure>

### UI Libraries

### Other Libraries

## Angular - Building Blocks

### Components

### Templates

### Directives

### Routing & Navigation

<https://angular.io/guide/router>

### Forms

### API (HttpClient)

### Modules

<https://angular.io/guide/ngmodules>

### Lazy Loading

<https://angular.io/guide/lazy-loading-ngmodules>

## Angular Testing & Debugging

### Error Management

<https://angular.io/errors/>

### Debugging

### Unit Tests / Integration Tests

### E2E Tests

### API Tests
